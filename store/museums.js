...
export const actions = {
  async index({ commit }, { destination, params }) {
    const { data, meta } = await this.$axios.$get(`destinations/${destination}/museums`,
      { params });
  ...
  async show({ commit }, { route_key, params }) {
    const { data } = await this.$axios.$get(`museums/${route_key}`,
      { params });
  ...
  async favorite({ commit }, { model, silent }) {
    ...
    const { data } = await this.$axios.$post(`museums/${model.route_key}/favorite`);
  ...
  async unfavorite({ commit }, { model, silent }) {
    ...
    const { data } = await this.$axios.$post(`museums/${model.route_key}/unfavorite`);
