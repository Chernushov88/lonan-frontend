...
export const actions = {
  async index({ commit }, { destination, params }) {
    const { data, meta } = await this.$axios.$get(`destinations/${destination}/shopping-stores`,
      { params });
  ...
  async show({ commit }, { route_key, params }) {
    const { data } = await this.$axios.$get(`shopping-stores/${route_key}`,
      { params });
  ...
  async favorite({ commit }, { model, silent }) {
    ...
    const { data } = await this.$axios.$post(`shopping-stores/${model.route_key}/favorite`);
  ...
  async unfavorite({ commit }, { model, silent }) {
    ...
    const { data } = await this.$axios.$post(`shopping-stores/${model.route_key}/unfavorite`);
