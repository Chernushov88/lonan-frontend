<template>
  <li class="item" :class="{'item--favored': item.is_liked}">
    <div class="item-wrap">
      ...
      <div class="item-body">
        <h4 class="item-title"><nuxt-link :to="localePath({name: 'destinations-destination-shopping-entity', params: {...$route.params, entity: item.id}})" v-text="item.title"></nuxt-link></h4>
  ...
  export default {
    ...
    props: {
      ...
      route: {
        required: false,
        default() {
          return {name: 'destinations-destination-shopping-entity', params: {...this.$route.params, entity: this.item.route_key}}
        }
      }
    },
    ...
    methods: {
      ...
      ...mapActions('shopping-stores', ['favorite', 'unfavorite']),
